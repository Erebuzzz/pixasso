...
- **MUST RULE: Compulsory Interactive Popup Discovery (`ask_question`)**: Before writing an implementation plan or generating component code, agents MUST call `ask_question` (or the host environment's interactive modal) to pop up clarifying questions to the user. Pure clarity must be achieved across theme/vibe, typography architecture (headline style, serif vs sans vs mono, weight, hierarchy), color/materiality, dimensionality (2D vs 2.5D vs 3D), and motion. Never skip this gate or bury questions in an unread plan document.
